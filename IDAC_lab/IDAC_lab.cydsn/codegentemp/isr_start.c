...
{
    CyRamVectors[CYINT_IRQ_BASE + isr_start__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + isr_start__INTC_NUMBER];
}
...
void isr_start_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((isr_start__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *isr_start_INTC_PRIOR = (*isr_start_INTC_PRIOR & (uint32)(~isr_start__INTC_PRIOR_MASK)) |
...
uint8 isr_start_GetPriority(void)
{
    ...
    priority = (*isr_start_INTC_PRIOR & isr_start__INTC_PRIOR_MASK) >> priorityOffset;
